...
%%%2. malicious users that always send the smallest finish time is not
%%%considered
function [Decision,SortedR,AvaiChanal,row,Trans] = EarliestFirst(D,R,S,SA)
    ...
    if(isempty(find(SA(:,t)==0))==0)
        %determine which channel is available
        [row]=find(~SA(:,t)); %row records the row number for each element==0
                   %similarly, row(1) is the first available channel
        AvaiChanal = numel(row); % available channel number
                   
        %determine which node has the earliest finish time
        SortedR=sortrows(R,4);%sort request R according to reqired finish time
        
        %determine the transmission time for this node
        for i=1:AvaiChanal
            Trans(i) = ceil(SortedR(i,3)/Cap(SortedR(i,2))); %transmission duration
        end
        
        %update the spectrum allocation matrix (update in the main program)
        %for i=1:AvaiChanal
        %    SA(row(i),t:t+Trans(i)) = SortedR(i,2);
        %end
        
        %update the Decision matrix
        for i=1:AvaiChanal
            Decision(SortedR(i,2),5)=1;
        end
    end
